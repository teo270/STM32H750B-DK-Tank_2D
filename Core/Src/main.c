...
//#include "FreeRTOS.h"
#include "stlogo.h"    // slikca za logo
...
int main(void)
{
  /* USER CODE BEGIN 1 */
	  ...
	  BSP_LED_Init(LED2);
	  BSP_LED_Init(LED3);
	  BSP_LED_On(LED3);  // Ledica je v obratni logiki, to naredi, da jo ugasneš, da te ne moti

	  // Initialize UART //
	  // Inicializacija UART
	  USART3_Init(&UART3Handle);
	  HAL_UART_Receive_IT(&UART3Handle, &uart3_rx_char, 1);
	  ...
	  UTIL_LCD_SetFuncDriver(&LCD_Driver); //Potrebno da touch screen dela
	  ...
	  Touch_Init(); // inicializacija touchscreen
	  ...
	  //DrawSprite_DMA2D(0, 0, 50, 50,(uint16_t*)Tank_Red);
	  //DrawSprite_DMA2D(100, 0, 50, 50,(uint16_t*)Tank_Blue);
	  //DrawSprite_DMA2D(200, 0, 50, 50,(uint16_t*)Tank_Orange);
	  //DrawSprite_DMA2D(100, 100, 10, 10,(uint16_t*)Square);
	  //LCD_DrawSprite565_DMA2D(0, 160, 91, 160, 90,(uint16_t*)welcome_screen);


	  /* počakaj malo, da vidiš original */
	  	  //DrawSprite_DMA2D(0, 11, 50, 50, Tank_Blue);

	  	//LCD_DrawSprite565_DMA2D(0, 80, 10, DST_SIZE, DST_SIZE, rotate_image(Tank_Red, 180.0f));



	      //HAL_Delay(3000);

	      //DrawSpriteRotated_DMA2D( 0, 0, 50, 50, (uint16_t*)Tank_Blue, 45.0f);
	 ...
	 // Display_String();
	  ...
	  while (1)
	  {
		  ...
		  //Touch_Process(touch_x_size, touch_y_size);      //main.c vidi deklaracijo (extern), compiler ve: “OK, touch_x_size je uint32_t, obstaja nekje.” //pri linkanju: linker poveže main.o z touch.o ,touch_x_size se veže na isto RAM lokacijo

		  Test_Game();


		  //DrawSpriteRotated_DMA2D( 160, 120, 50, 50, (uint16_t*)Tank_Blue, angle);
		  //LCD_DrawSprite565_DMA2D(0, 200, 10, DST_SIZE, DST_SIZE, rotate_image(Tank_Red, angle));

		  //LCD_DrawSprite565_DMA2D(0, 200, 200, DST_SIZE, DST_SIZE, rotate_image_bilinear(Tank_Blue, angle));
